...
func main() {
	...
	//container to store accuracy values
	accuracyList := make([]float64, nRuns)
	...
	//Print accuracy values for every run
	for i, accuracy := range accuracyList {
		fmt.Printf("Run %d: Accuracy = %.2f%%\n", i+1, accuracy*100)
	}
...
func RunLinReg(dis []float64, mv []float64, nRuns int) Response {
	...
	for i := 0; i < nRuns; i++ {
		...
		fmt.Printf("Run %d - Coefficients: %v\n", i+1, LinRegresponse.Coefficients)
	}
...
// MinMaxCoordinates for EquationLine(LinReg)
